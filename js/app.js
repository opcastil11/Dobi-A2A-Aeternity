...
const CONTRACT_ACI_URL    = '/AIPaymentManager.aci.json'; // ACI precompilado (preferido)
...
async function getContract() {
  ...
  // 1) intenta con ACI (preferido)
  try {
    const aci = await (await fetch(CONTRACT_ACI_URL, { cache: 'no-store' })).json();
    if (!aci?.functions) throw new Error('ACI inválido');
    ct = await aeSdk.initializeContract({ aci, address: CONTRACT_ADDRESS });
    // sanity check
    if (!ct.methods) throw new Error('Contract instance sin methods (ACI)');
    return ct;
  } catch (e) {
    console.warn('[contract] ACI no disponible o inválido, fallback a source:', e?.message || e);
  }

  // 2) fallback: usa source + compiler (ya tienes onCompiler)
  const res = await fetch(CONTRACT_SOURCE_URL, { cache: 'no-store' });
  ...
  if (!ct.methods) throw new Error('Contract instance sin methods (source)');
  return ct;
}
...
async function scEnsureCharger(st) {
  ...
  if (!c?.methods?.get_charger_info) {
    agentEvent('SC: contrato sin ACI/métodos. Revisa ACI o compiler.');
    return;
  }
...
async function scEnsureProvider(payeeAddr, priceAEkWh) {
  const c = await getContract();
  if (!c?.methods?.get_provider_info) {
    agentEvent('SC: contrato sin ACI/métodos. Revisa ACI o compiler.');
...
async function agentStartSession(st) {
  ...
  try {
    ...
    if (!c?.methods?.start_consumption) {
      agentEvent('SC: contrato sin ACI/métodos. Revisa ACI o compiler.');
      return;
    }
  ...
  state.agent.mqtt = setInterval(async () => {
    ...
    if (sessId != null) {
      try {
        ...
        if (!contract?.methods?.update_consumption_reading) {
          agentEvent('SC: contrato sin ACI/métodos. Revisa ACI o compiler.');
          return;
        }
        const kwhInt = kwhToInt(deltaKwh);
        await contract.methods.update_consumption_reading(sessId, kwhInt);
    ...
    if (state.agent.consumed >= state.agent.session.kwhGoal) {
      ...
      try {
        ...
        if (state.agent.session.id != null) {
          const contract = await getContract();
          if (!contract?.methods?.stop_consumption) {
            agentEvent('SC: contrato sin ACI/métodos. Revisa ACI o compiler.');
            return;
          }
          await contract.methods.stop_consumption(state.agent.session.id, totalIntKwh);
