...
import { decode, Encoding } from './encoder.js';
...
export function isAddressValid(maybeEncoded, ...encodings) {
...
export function ensureEncoded(maybeEncoded, ...encodings) {
  decode(maybeEncoded);
...
export function isEncoded(maybeEncoded, ...encodings) {
  try {
...
export function hash(input) {
  return _Buffer.from(blake2b(input, undefined, 32)); // 256 bits
}
...
export function encodeVarUInt(value) {
  ...
  if (value <= 0xffff) {
    return concatBuffers([_Buffer.from([0xfd]), _Buffer.from(new Uint16Array([value]).buffer)]);
  }
  if (value <= 0xffffffff) {
    return concatBuffers([_Buffer.from([0xfe]), _Buffer.from(new Uint32Array([value]).buffer)]);
  }
  return concatBuffers([_Buffer.from([0xff]), _Buffer.from(new BigUint64Array([BigInt(value)]).buffer)]);
}
const messagePrefix = _Buffer.from('aeternity Signed Message:\n', 'utf8');
 ...
 * @category utils
 */
export function verifyMessageSignature(message, signature, address) {
